...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("```Petercord, Mohon Balas Ke Pesan Pengguna atau ketik .deteksi (ID/Username) Yang mau Anda deteksi```")
        return
    if input_str:
        try:
            uid = int(input_str)
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await steal.reply(
                "```petercord Mohon Unblock @tgscanrobot Dan Coba Lagi```"
            )
