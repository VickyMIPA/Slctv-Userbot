...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**`ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ`{ALIVE_NAME} Sedang AFK!**",
    f"**`ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ` User {ALIVE_NAME} Sedang AFK\n `Tunggu Sampai Dia Kembali Online! Jangan rindu ya berat`**",
...
async def set_afk(afk_e):
    ...
    if user.last_name:
        ...
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="【ᴘᴇᴛᴇʀᴄᴏʀᴅ】"))
    if BOTLOG:
        await afk_e.client.send_message(BOTLOG_CHATID, "🔮AFK\n**𝗣𝗘𝗡𝗚𝗚𝗨𝗡𝗔 𝗣𝗘𝗧𝗘𝗥𝗖𝗢𝗥𝗗 Telah AFK!**")
    ISAFK = True
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("【ᴘᴇᴛᴇʀᴄᴏʀᴅ】"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**𝗣𝗘𝗡𝗚𝗚𝗨𝗡𝗔 𝗣𝗘𝗧𝗘𝗥𝗖𝗢𝗥𝗗 Telah Kembali! Emang gada yang rindu!**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**🛡 𝗣𝗘𝗡𝗚𝗚𝗨𝗡𝗔 𝗣𝗘𝗧𝗘𝗥𝗖𝗢𝗥𝗗 {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☛ **𝗔𝗟𝗔𝗦𝗔𝗡:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**🛡 𝗣𝗘𝗡𝗚𝗚𝗨𝗡𝗔 𝗣𝗘𝗧𝗘𝗥𝗖𝗢𝗥𝗗 Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **𝗔𝗟𝗔𝗦𝗔𝗡:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"🛡 **𝗣𝗘𝗡𝗚𝗚𝗨𝗡𝗔 𝗣𝗘𝗧𝗘𝗥𝗖𝗢𝗥𝗗 Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☛ **𝗔𝗟𝗔𝗦𝗔𝗡**: `{AFKREASON}`")
                else:
