...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**`➥ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ` {ALIVE_NAME} ➥sᴇᴅᴀɴɢ ᴀꜰᴋ**",
    f"**`➥ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ` {ALIVE_NAME} ➥sᴇᴅᴀɴɢ ᴀꜰᴋ\n `ᴛᴜɴɢɢᴜ sᴀᴍᴘᴀɪ ᴅɪᴀ ᴋᴇᴍʙᴀʟɪ ᴏɴʟɪɴᴇ!`\n `ᴊᴀɴɢᴀɴ sᴀɴɢᴇ ʟᴏ!`**",
    f"**`➥ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ` {ALIVE_NAME} ➥sᴇᴅᴀɴɢ ᴀꜰᴋ\n `ᴛᴜɴɢɢᴜ sᴀᴍᴘᴀɪ ᴅɪᴀ ᴋᴇᴍʙᴀʟɪ ᴏɴʟɪɴᴇ!`**",
    f"**`➥ᴍᴀᴀꜰ ᴘᴇɴɢɢᴜɴᴀ` {ALIVE_NAME} ➥sᴇᴅᴀɴɢ ᴀꜰᴋ**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**ᴀꜰᴋ**\n**➥ᴘᴇɴɢɢᴜɴᴀ ᴛᴇʟᴀʜ ᴏꜰꜰʟɪɴᴇ**\
        \n☛ **Alasan:** `{string}`")
    else:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**𝗼𝗻𝗹𝗶𝗻𝗲🔥**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"*➥ᴍᴀᴀꜰ {ALIVE_NAME}  sᴇᴅᴀɴɢ ᴀꜰᴋ** {afk_since} **ʏᴀɴɢ ʟᴀʟᴜ.**\
                        \n☛ **ᴀʟᴀsᴀɴ:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**➥ᴍᴀᴀꜰ sᴇᴅᴀɴɢ ᴀꜰᴋ** {afk_since} **ʏᴀɴɢ ʟᴀʟᴜ.**\
                            \n☛ **ᴀʟᴀsᴀɴ:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"➥ **ᴍᴀᴀꜰ sᴇᴅᴀɴɢ ᴀꜰᴋ** {afk_since} **ʏᴀɴɢ ʟᴀʟᴜ**.\
                        \n☛ **ᴀʟᴀsᴀɴ**: `{AFKREASON}`")
                else:
