...
async def _(fry):
    await fry.edit("`Ownerku Mengaktifkan Kekuatan Telegram...💦`")
    level = fry.pattern_match.group(2)
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`Mohon Balas Di Sticker Slctv`")
        return
    ...
    async with fry.client.conversation(chat) as conv:
        try:
            ...
            await fry.reply("`Slctv Mohon Unblock` @image_deepfrybot`...`")
            return
        if response.text.startswith("Forward"):
            await fry.edit("`Slctv Mohon Matikan Setelan Forward Privasi...`")
        else:
...
async def _(fry):
    ...
    async with fry.client.conversation(chat) as conv:
        try:
            ...
            return
        if response.text.startswith("Forward"):
            await fry.edit("`Slctv Mohon Matikan Setelan Privasi Forward...`")
        else:
            downloaded_file_name = await fry.client.download_media(
                response.media,
                TEMP_DOWNLOAD_DIRECTORY
